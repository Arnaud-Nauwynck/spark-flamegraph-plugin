...
/**
 * registry for Thread StackTrace
 *
 * use to represent a stack trace with an id
 */
@Slf4j
public class StackTraceEntryRegistry {
    ...
    public PartiallyResolvedStackTrace finishPartialResolveStackTrace(
    ...
    public StackTraceEntry[] localResolveOrRegisterStackTrace(ThreadInfo threadInfo) {
        synchronized (lock) {
            val stackTrace = threadInfo.getStackTrace();
            final int len = stackTrace.length;
            StackTraceEntry[] res = new StackTraceEntry[len];
            StackTraceEntry curr = rootEntry;
            for (int i = len - 1; i >= 0; i--) {
                val key = (i != 0) ? StackTraceElementKey.createFrom(stackTrace[i])
                        : StackTraceElementKey.createFrom0(stackTrace[i], threadInfo.getThreadState(), threadInfo.getLockInfo());
                StackTraceEntry foundChild = curr.findChild(key);
                if (foundChild == null) {
                    // register new child
                    int childId = idGenerator++;
                    foundChild = new StackTraceEntry(childId, curr, key);
                    byId.put(childId, foundChild);
                    curr._registerChild(foundChild);
                }
                res[i] = foundChild;
